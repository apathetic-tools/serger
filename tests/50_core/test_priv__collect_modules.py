# tests/50_core/test_priv__collect_modules.py
...
class TestCollectModulesBasic:
    ...
    def test_collect_simple_modules(self, tmp_path: Path) -> None:
    ...
    def test_collect_with_external_imports(self, tmp_path: Path) -> None:
    ...
    def test_collect_removes_internal_imports(self, tmp_path: Path) -> None:
        """Should remove internal imports from module bodies."""
        src_dir = tmp_path

        (src_dir / "a.py").write_text("from testpkg.b import something\n\nA = 1\n")

        file_paths, package_root, file_to_include = _setup_collect_test(src_dir, ["a"])
    ...
    def test_collect_skip_missing_module(self, tmp_path: Path) -> None:
    ...
    def test_collect_strips_shebangs(self, tmp_path: Path) -> None:
    ...
    def test_collect_preserves_order(self, tmp_path: Path) -> None:
        ...
        c_pos = next(
            i for i, p in enumerate(parts) if "# === c" in p or "# === c.py ===" in p
        )
        a_pos = next(
            i for i, p in enumerate(parts) if "# === a" in p or "# === a.py ===" in p
        )
        b_pos = next(
            i for i, p in enumerate(parts) if "# === b" in p or "# === b.py ===" in p
        )
...
class TestCollectModulesImportHandling:
    ...
    def test_preserves_import_order(self, tmp_path: Path) -> None:
    ...
    def test_handles_multiline_imports(self, tmp_path: Path) -> None:
...
class TestCollectModulesSources:
    ...
    def test_returns_module_sources_dict(self, tmp_path: Path) -> None:
    ...
    def test_sources_match_original_content(self, tmp_path: Path) -> None:
    ...
    def test_parts_format(self, tmp_path: Path) -> None:
...
class TestCollectModulesMultiPackage:
    ...
    def test_collect_multi_package_detects_all_packages(self, tmp_path: Path) -> None:
    ...
    def test_collect_multi_package_three_packages(self, tmp_path: Path) -> None:
