# tests/50_core/test_execute_post_processing.py
...
def test_execute_post_processing_with_ruff(tmp_path: Path) -> None:
    ...
    config = make_post_processing_config_resolved(
        ...
        categories={
            "formatter": make_post_category_config_resolved(
                ...
                priority=["ruff"],
                tools={
                    "ruff": make_tool_config_resolved(args=["format"], command="ruff"),
                },
            ),
        },
    )
...
def test_execute_post_processing_disabled(tmp_path: Path) -> None:
...
def test_execute_post_processing_priority_fallback_on_failure(
    ...
    config = make_post_processing_config_resolved(
        ...
        categories={
            "formatter": make_post_category_config_resolved(
                ...
                tools={
                    "tool1": make_tool_config_resolved(args=["format"], command="ruff"),
    ...
    assert "ruff" in str(executed_commands[0]) or "tool1" in str(executed_commands[0])
    assert "black" in str(executed_commands[1]) or "tool2" in str(executed_commands[1])
