# tests/9_integration/test_zero_config_auto_detect.py
...
def test_zero_config_auto_detect_package_not_found_in_module_bases(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
"""
    ...
    # Create config with package that doesn't exist in module_bases
    config = tmp_path / f".{mod_meta.PROGRAM_CONFIG}.json"
    config.write_text(
        """{
    "package": "nonexistent",
...
def test_zero_config_no_auto_detect_when_all_bases_have_multiple_modules(
    ...
    # Create config with multiple module_bases, all with multiple modules
    config = tmp_path / f".{mod_meta.PROGRAM_CONFIG}.json"
    config.write_text(
        """{
...
def test_zero_config_auto_detect_with_custom_module_bases(
    ...
    """Build succeeds with custom module_bases when single module exists.

    Scenario:
    - Has config with custom module_bases=["lib"] but no package/includes
    ...
    # Create single package directory in custom module_bases
...
"""
    ...
    # Create config with custom module_bases but no package/includes
    config = tmp_path / f".{mod_meta.PROGRAM_CONFIG}.json"
    config.write_text(
        """{
    "module_bases": ["lib"],
...
def test_zero_config_auto_detect_picks_first_base_with_single_module(
    ...
    """Build succeeds by picking first module_base with exactly 1 module.
...
"""
    ...
    # Create config with multiple module_bases but no package/includes
    config = tmp_path / f".{mod_meta.PROGRAM_CONFIG}.json"
    config.write_text(
        """{
    "module_bases": ["src", "lib"],
